...
- 引数として渡されたフォルダー名を **`Reports/` 直下** のフォルダーとして特定する（例：`Reports/202604-HANNOVER/`）
- `Reports/` 直下に見つからない場合は、リポジトリ全体を `find` で探索してから判断する
...
- **make-report は「写真・動画審査のみモード」で実行する**（Report.md は変更しない）
- このモードでの審査範囲は以下に限る（差分ビルド原則「確定済み写真を再審査しない」を優先する）：
  - 動画ファイルの残存チェック（`find Images -iname "*.mp4" -o -iname "*.mov" ...` 1回）→ 残っていればサムネイル抽出・削除を実行
  - 未分類ファイルの有無確認（`Images/` 直下・`OtherPictures/`・`unUsed/` のいずれにも該当しない迷子ファイルがないか `ls` で確認）
  - **既存の分類済み写真を Read で開き直す再審査はしない**
...
- 差分ビルドモード → **変更ファイルリストに載っている新規・変更ファイルのみ**（確定済み写真の再審査はしない）
- 「変更なし」判定時 → **動画残存チェックと未分類ファイル確認のみ**（完了済み判定の章を参照。既存写真の Read 再審査はしない）
...
作業対象フォルダー直下（例：`Reports/202604-HANNOVER/build_log.md`）に作成・追記する。
毎回作り直さず、履歴として追記する。
