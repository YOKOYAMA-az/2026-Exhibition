...
### 9.4 最終更新列・★/◎/△バッジの自動更新（過去24時間／3日／7日以内の実質更新）

README.md の「出張報告書」テーブル・「講演会レポート」テーブル・「Strategy」テーブルに、実質的な更新を示す**相対時間テキスト**とバッジを付ける。**表の一番左に `最終更新`（相対時間テキスト）列、その次にバッジ専用列を追加する**。バッジ記号だけでは「何日前か」が分からないため、必ずテキストの相対時間を左端に添える（リンクの後ろに文字として埋め込むと、行によってリンクの長さが違うため位置が揃わず見づらい。専用列にすることで縦に並び、一目で分かる）。
...
バッジ列の見出しは空欄でよい（`|  |` とし、ヘッダー行も `|:---:|` とする）。`最終更新` 列の表記ルール（`Now`/`Xmin ago`/`Today`/`Yesterday`/`X days ago`/`1 week ago`等）は archive-report.md の「最終更新列の表記ルール」節と共通。今回処理中のレポートだけでなく、**3テーブルすべての全行を毎回洗い直す**（stale化防止のため）。凡例がなければテーブル直前に `> 最終更新列は **YYYY-MM-DD HH:MM** 時点の相対時間です。★ = 過去24時間以内に実質的な更新があったもの　◎ = 過去3日以内（24時間超）に実質的な更新があったもの　△ = 過去7日以内（3日超）に実質的な更新があったもの` を追加する。

```markdown
| 最終更新 |  | 日付 | 出張先 | 参加者 | 写真枚数・容量 | ナレッジ化 |
|:---:|:---:|---|---|---|:---:|:---:|
| Yesterday | ★ | 2026年4月24日 | [🇫🇷 フランス／IMS Manutention...](Reports/202604-IMS/Report.md) | 山崎・橋本GM | 21枚・2.7MB | [7/3・7/7](Reports/archive_log.md) |
| 2 days ago | ◎ | 2026年4月17日 | [🇺🇸 米国／BIC...](Reports/202604-BIC/Report.md) | 山崎・橋本GM | 28枚・4.0MB | [7/8](Reports/archive_log.md) |
| 1 week ago |  | 2026年4月20日 | [🇩🇪 ドイツ／ハノーバーメッセ...](Reports/202604-HANNOVER/Report.md) | 山崎・橋本GM | 34枚・4.5MB | [7/2](Reports/archive_log.md) |
| — |  | 2025年1月31日 | [🇯🇵 日本／福島トヨペット...](Reports/202501-FukushimaToyopet/Report.md) | 山崎・廣田GM | 10枚・1.7MB | — |
```

`PUBLISH_SUMMARY.md` が存在しない行（このパイプラインを一度も通っていないレポート）は、最終更新列を `—`、バッジ列を空欄にする。
...
- 24時間以内 → `最終更新`列に `Today`、バッジ列に `★`
- 24時間超・3日以内 → `最終更新`列に `X days ago`、バッジ列に `◎`
- 3日超・7日以内 → `最終更新`列に `X days ago`、バッジ列に `△`
- 7日超・14日未満 → `最終更新`列に `1 week ago`、バッジ列は空欄
- 14日以上 → `最終更新`列に `X weeks ago`、バッジ列は空欄
- `PUBLISH_SUMMARY.md` が存在しない → `最終更新`列に `—`、バッジ列は空欄
...
#### Strategy・講演会レポートテーブル：git履歴から「一括操作を除いた」実質的な最終編集日を求める

Strategy配下・講演会レポート（`Reports/講演フォルダ/*.md`）のファイルはpublishパイプラインを通らないため `PUBLISH_SUMMARY.md` がない。どちらも同じロジックでgit ログから求める。**フォルダ移動・一括リネームのような機械的な一括コミットは実質的な編集とみなさない**。
...
# Strategyテーブル・講演会レポートテーブルに掲載されている全ファイルに対して実行する
for f in $(grep -oE '\]\([^)]+\.md\)' README.md | sed -n '/## Strategy/,$p' 2>/dev/null); do :; done
# ↑ README.mdの「## Strategy」「## 講演会レポート」各セクション内のリンク先を対象に、以下を1ファイルずつ実行する
t=$(find_real_last_edit "対象ファイルパス")
[ -z "$t" ] && continue
days=$(( (today - t) / 86400 ))
```

- 全履歴が一括操作のみで、実質的な編集コミットが1件も見つからない場合は `最終更新`列にraw `git log --follow -1` のタイムスタンプ由来の相対時間をフォールバック表示し、バッジ列は空欄にする（安全側に倒す。archive-report.mdの`rel_time()`と同じフォールバック方針）
- 判定・付与ルールは出張報告書テーブルと同じ（`最終更新`列に相対時間テキスト、バッジ列に24時間以内→`★`、24時間超・3日以内→`◎`、3日超・7日以内→`△`、それ以外→空欄）
