...
このリポジトリでは過去に733ファイル・1454ファイル等を同時に変更する一括リファクタリングコミットがあり、単純に「そのファイルを最後に触ったコミット」を使うと、内容と無関係にほぼ全ファイルが「最近更新」と誤判定される。これを避けるため、**既知の機械的操作のコミットメッセージパターンを除外**し、加えて**極端に大きい一括コミット（100ファイル超）**も安全側で除外する（`archive-report.md`の🆕バッジ判定ロジックと同一の関数を使う。詳細・除外パターンの追記方針は `archive-report.md`「🆕バッジ列」節を参照）。
